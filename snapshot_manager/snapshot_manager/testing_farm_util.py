...
class TestingFarmRequest:
    ...
    def watch(self) -> tuple["TestingFarmWatchResult", str]:
        ...
        cmd = f"testing-farm watch --no-wait --format json --id {self.request_id}"
        ...
        try:
            ...
            watch_result, artifacts_url = TestingFarmWatchResult.from_json(stdout)
...
@dataclasses.dataclass(kw_only=True, unsafe_hash=True)
class TestingFarmWatchResult:
    @enum.unique
    class OverallState(enum.StrEnum):
        ERROR = "error"
        PASSED = "passed"

        @classmethod
        def all(cls) -> list:
            return [s for s in TestingFarmWatchResult.OverallState]

        @classmethod
        def is_valid(cls, string: str) -> bool:
            return string in cls.all()
    ...
    class State(enum.StrEnum):  #
        ...
        @classmethod
        def all(cls) -> list:
            return [s for s in TestingFarmWatchResult.State]

        @classmethod
        def is_valid(cls, string: str) -> bool:
            return string in cls.all()

    overall: OverallState = None
    """The overall state of the testing-farm request"""

    state: State = None
    """The state itself"""

    artifacts: str = None
    """The artifacts URL pointing to the testing-farm results"""
    ...
    def to_icon(self) -> str:
        ...
        if self.overall == self.OverallState.RUNNING:
            return ":running:"
        return ":grey_question:"
        # elif self.state == self.REQUEST_WAITING_TO_BE_QUEUED:
        #     return ":hourglass:"
        # elif self.state == self.REQUEST_QUEUED:
        #     return ":inbox_tray:"
        # elif self.state == self.TESTS_PASSED:
        #     return ":white_check_mark:"
        # elif self.state == self.TESTS_FAILED:
        #     return ":x:"
        # elif self.state == self.TESTS_ERROR:
        #     return ":x:"
        # elif self.state == self.TESTS_UNKNOWN:
        #     return ":grey_question:"
        # elif self.state == self.PIPELINE_ERROR:
        #     return ":warning:"
    ...
    def is_complete(self) -> bool:
        ...
        >>> TestingFarmWatchResult(overall="complete").is_complete
        True

        >>> TestingFarmWatchResult(overall="running").is_complete
        False
        """
        return self.overall in [self.OverallState.COMPLETE]
    ...
    def is_error(self) -> bool:
        ...
        >>> TestingFarmWatchResult(overall="complete", state="error").is_complete
        True

        >>> TestingFarmWatchResult(overall="complete", state="passed").is_complete
        True
    ...
    def expects_artifacts_url(self) -> bool:
    ...
    @classmethod
    def from_json(cls, string: str) -> tuple["TestingFarmWatchResult", str]:
        """Inspects the JSON output of a testing-farm watch call and returns a tuple of result and artifacts url (if any).
        ...
        >>> s='''
        ... {
        ...  "id": "271a79e8-fc9a-4e1d-95fe-567cc9d62ad4",
        ...  "state": "complete",
        ...  "artifacts": "http://artifacts.osci.redhat.com/testing-farm/271a79e8-fc9a-4e1d-95fe-567cc9d62ad4",
        ...  "overall": "error",
        ...  "arches_requested": [
        ...    "x86_64"
        ...  ],
        ...
        ... }
        ... '''
        >>> TestingFarmWatchResult.from_json(s)
        TestingFarmWatchResult(overall='error', state='complete', artifacts='http://artifacts.osci.redhat.com/testing-farm/271a79e8-fc9a-4e1d-95fe-567cc9d62ad4')
        """
        watch_result_json = json.loads(string)

        watch_result = TestingFarmWatchResult()

        # Get overall field
        if "overall" not in watch_result_json:
            raise KeyError(f"failed to find 'overall' key in JSON: {string}")
        overall_state = watch_result_json["overall"]
        if not cls.OverallState.is_valid(overall_state):
            raise ValueError(
                f"'overall' value: '{overall_state}' is not part the overall states {cls.OverallState.all()}"
            )
        watch_result.overall = overall_state
        # Get state
        if "state" not in watch_result_json:
            raise KeyError(f"failed to find 'state' key in JSON: {string}")
        state = watch_result_json["state"]
        if not cls.State.is_valid(state):
            raise ValueError(
                f"'state' value: '{state}' is not part the states {cls.State.all()}"
            )
        watch_result.state = state

        if watch_result.expects_artifacts_url():
            if "artifacts" not in watch_result_json:
                raise KeyError(f"failed to find 'artifacts' key in JSON: {string}")
            watch_result.artifacts = watch_result_json["artifacts"]
